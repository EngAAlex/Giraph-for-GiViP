...
public class SendMessageCache<I extends WritableComparable, M extends Writable>
  ...
  //ADDED VARIABLE
  /** MessagesManager for messages saving */
  private MessagesSniffer msgSniffer;  
  /**
   * Constructor
   *
  ...
  public SendMessageCache(ImmutableClassesGiraphConfiguration conf,
    ...
    msgSniffer = serviceWorker.getMessagesSniffer();
  }
  ...
  public void sendMessageRequest(I destVertexId, M message) {
    ...
    if (workerMessageSize >= maxMessagesSizePerWorker) {
      ...
      //Catch total messages number 
      int exchangedMessages = 0;
      PairList<Integer, VertexIdMessages<I, M>>.Iterator iterator = workerMessages.getIterator();
            
      while(iterator.hasNext()){
        iterator.next();
        VertexIdMessageIterator<I, M> messagesIterator = iterator.getCurrentSecond().getVertexIdMessageIterator();
      ...
      // Wrap the data for MessagesManager 
      int workerTaskSourceId =
      this.clientProcessor.getServiceWorker().getWorkerInfo().getTaskId();
      int workerTaskDestId = workerInfo.getTaskId();
      boolean isInternal = workerTaskSourceId == workerTaskDestId;
  ...
  public void flush() {
    ...
    PairList<Integer, VertexIdMessages<I, M>>.Iterator nestedIterator;
    
    int workerTaskSourceId = this.getServiceWorker().getWorkerInfo().getTaskId(); //Current worker
    int workerTaskDestId;
    int exchangedMessages=0;
    ...
    while (iterator.hasNext()) {
      ...
      nestedIterator = iterator.getCurrentSecond().getIterator();  
      workerTaskDestId = iterator.getCurrentFirst().getTaskId();   //Worker to send messages to.
      
      while (nestedIterator.hasNext()) {
        nestedIterator.next();

        // Wrap the data for MessagesManager
        VertexIdMessageIterator<I, M> messagesIterator =
            nestedIterator.getCurrentSecond().getVertexIdMessageIterator();
        while (messagesIterator.hasNext()) {
          messagesIterator.next();
          exchangedMessages++;
        }
      ...
      try {
        this.msgSniffer.newExchangedMessage(
          workerTaskSourceId, workerTaskDestId, exchangedMessages,
          writableRequest.getSerializedSize(),
          workerTaskSourceId == workerTaskDestId);
      } catch (IOException e) {
        e.printStackTrace();
      }
