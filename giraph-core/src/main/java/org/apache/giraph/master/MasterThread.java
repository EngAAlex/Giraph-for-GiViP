...
import org.apache.giraph.profiler.HierarchyWrapper;
import org.apache.giraph.profiler.SuperstepInfoCatcher;
  ...
  public void run() {
    ...
    try {
      ...
      if (bspServiceMaster.becomeMaster()) {
        ...
        //ADDED CODE
        // First capture of hierarchy tree of workers
        List<WorkerInfo> workers = Lists.newArrayList();
        ...
        for(WorkerInfo worker: workers){
          ...
          hWrapper.saveNodeHierarchy(node,worker);
//          LOG.info("Initial capture of workers with TaskId: "+worker.getTaskId());
        }
...
//        sCatcher.localWrite();
        sCatcher.writeToHDFS(); //write information in HDFS
