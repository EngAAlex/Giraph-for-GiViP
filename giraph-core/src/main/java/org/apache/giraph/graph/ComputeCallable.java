...
public class ComputeCallable<I extends WritableComparable, V extends Writable,
  ...
  public Collection<PartitionStats> call() {
    ...
    //ADDED CODE
    // Set current superstep in ExecutedSuperstepWorkerInfo 
    serviceWorker.getSuperstepInfo().updateSuperstep(graphState.getSuperstep());
    long millis = 0;
    if (LOG.isInfoEnabled()) {
        millis = Times.getNanosSince(TIME, startNanos) /
          Time.NS_PER_MS;
      LOG.info("call: Computation took " + millis + " secs for "  +
          partitionStatsList.size() + " partitions on superstep " +
          graphState.getSuperstep() + ".  Flushing started");
    }
    
    //Get all data for ExecutedSuperstepWorkerInfo
    int nPartitions = partitionStatsList.size();
    String computName = configuration.getComputationName();
    int prVertices = 0;
    for(PartitionStats p: partitionStatsList){
        prVertices += p.getVertexCount();
    }
    
    serviceWorker.getSuperstepInfo().newSuperstepInfo(computName, millis, prVertices, nPartitions);
  ...
  private PartitionStats computePartition(
    ...
    synchronized (partition) {
      for (Vertex<I, V, E> vertex : partition) {
        ...
        //LOG to wrap data targeted to debug phase
//        LOG.info("mariaDebuggingLogs: vertexId "+vertex.getId()+
//        " is owned by workerTaskId :"
//            +" "+
